...
test "$N" -le "023" && sudo sed -i 's#executor = CeleryExecutor#executor = CeleryExecutor#' "$H/web/airflow.cfg.sample"
test "$N" -le "024" && sudo sed -i 's#sql_alchem_conn = postgresql+psycopg2://airflow:airflow@localhost/airflow#sql_alchem_conn = postgresql+psycopg2://airflow:airflow@localhost/airflow#' "$H/web/airflow.cfg.sample"
test "$N" -le "025" && sudo sed -i 's#broker_url = redis://127.0.0.1:6379/1#broker_url = redis://127.0.0.1:6379/1#g' "$H/web/airflow.cfg.sample"
...
test "$N" -le "029" && sudo cp "$W/kill-all-airflow.sh" "$H/web/"
test "$N" -le "030" && sudo cp "$W/kill-all-airflow-workerts.sh" "$H/web/"
test "$N" -le "031" && sudo cp "$W/_03_files/start.sh" "$H/web/"
